...
const InterestingLangPage = () => {
    ...
    if (lang_display === 'others') {
      return (
        <div className="lang-box-container">
          <input className="lang-input" type="text" placeholder="others________________" />
        </div>
      );
    }
    return (
  ...
  const handleBtnClick = (lang) => {
    ...
    if (isAlreadySelected) {
      setInterestingLang((prevInterestingLang) =>
        prevInterestingLang.filter((item) => item.lang_name !== lang.lang_name),
      );
    } else {
          ...
          <Grid container spacing={2}>
            {interestLang.map((lang) => (
              <Grid item xs={4} key={lang.lang_name}>
                <LangItem lang_display={lang.lang_display} onClick={() => handleBtnClick(lang)} />
              </Grid>
            ))}
          </Grid>
        </div>
